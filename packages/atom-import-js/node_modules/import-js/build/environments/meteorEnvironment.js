...
const coreModules = ['meteor/accounts-base', 'meteor/blaze', 'meteor/check', 'meteor/ddp-client', 'meteor/ddp-rate-limiter', 'meteor/ejson', 'meteor/email', 'meteor/http', 'meteor/check', 'meteor/meteor', 'meteor/mongo', 'meteor/random', 'meteor/reactive-var', 'meteor/session', 'meteor/templating', 'meteor/tracker'];
...
function meteorPackageDependencies({ config }) {
  const meteorPackagesPath = _path2['default'].join(config.workingDirectory, '.meteor/packages');
  ...
  const packages = (_fs2['default'].readFileSync(meteorPackagesPath, 'utf8')
  ...
  map(pkg => 'meteor/' + String(pkg.trimLeft()))
...
function meteorPackageVersions(projectRootDir) {
  ...
  const meteorVersionsPath = _path2['default'].join(projectRootDir, '.meteor/versions');
  ...
  const pkgVersionPairs = _fs2['default'].readFileSync(meteorVersionsPath, 'utf8').match(/^[^@\s]+@[^\s]+$/gm) || [];
  ...
  pkgVersionPairs.forEach(pkgVersionPair => {
    ...
    const pkg = _pkgVersionPair$split2[0],
...
function extractExportsFromMeteorPackage(projectRootDir, pkg, pkgVersion) {
  ...
  let isopackRoot = _path2['default'].join(_os2['default'].homedir(), '.meteor/packages', pkg.replace(':', '_'), pkgVersion);
  let isopackPath = _path2['default'].join(isopackRoot, 'isopack.json');
  let isopack = _FileUtils2['default'].readJsonFile(isopackPath);
  ...
  isopackVer.builds.forEach(build => {
    const buildIsopackPath = _path2['default'].join(isopackRoot, build.path);
    const buildIsopack = _FileUtils2['default'].readJsonFile(buildIsopackPath);
    ...
    if (buildIsopack.resources) {
      const mainModuleResource = buildIsopack.resources.find(resource => resource.fileOptions && resource.fileOptions.mainModule === true);

      if (mainModuleResource) {
        const pathToRequiredFile = _path2['default'].join(isopackRoot, mainModuleResource.file);
        const requiredFileContent = _fs2['default'].readFileSync(pathToRequiredFile, 'utf8');
        ...
        const named = _findExports.named;
...
function meteorPackageNamedExports({ config }) {
  ...
  meteorPackageDependencies({ config }).forEach(meteorPkg => {
    ...
    const extractedExports = extractExportsFromMeteorPackage(config.workingDirectory, pkg, pkgVersion);
...
exports['default'] = {
  ...
  moduleSideEffectImports({ pathToCurrentFile, config }) {
    ...
    ['.html', '.css'].forEach(ext => {
      const moduleSpecifier = '' + String(basePath) + String(ext);
  ...
  packageDependencies({ config }) {
    const npmPackages = (0, _findPackageDependencies2['default'])(config.workingDirectory, config.get('importDevDependencies'));

    return new Set([...meteorPackageDependencies({ config }), ...Array.from(npmPackages)]);
  }
};
