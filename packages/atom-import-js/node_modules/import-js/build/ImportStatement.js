...
class ImportStatement {
  ...
  _defaultImportString(maxLineLength, tab) {
    ...
    const equals = _equalsAndValue.equals,
          value = _equalsAndValue.value;


    const line = String(this.declarationKeyword || '') + ' ' + String(this.defaultImport || '') + ' ' + String(equals) + ' ' + String(value);
  ...
  _namedImportString(maxLineLength, tab) {
    ...
    const equals = _equalsAndValue2.equals,
    ...
    const namedOneLine = '{ ' + String(named.join(', ')) + ' }';
    const line = String(this.declarationKeyword || '') + ' ' + prefix + namedOneLine + ' ' + String(equals) + ' ' + String(value);
    if (!isLineTooLong(line, maxLineLength)) {
      return line;
    }

    const tabJoined = named.join(',\n' + String(tab));
    const namedMultiLine = '{\n' + String(tab) + String(tabJoined) + (this.danglingCommas ? ',' : '') + '\n}';
