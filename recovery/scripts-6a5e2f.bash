...
function importWord {
  ...
  cd ${EHI_PROJECT_DIR}/mse-ui
  local res=$(importjs word $word $file | sed -n -e '/^{"messages":/p')
  
  echo $res | jq -r ".fileContent"
  local contents=$($(echo $res) | jq -r ".fileContent")
  local messages=$($(echo $res) | jq -r ".messages[]")
