...
function resolveNestedNamedExports(node, absolutePathToFile) {
  ...
  if (node.type === 'CallExpression' && node.callee.name === 'require' && node.arguments.length === 1 && node.arguments[0].type === 'StringLiteral') {
    ...
    const pathToRequiredFile = _requireRelative2['default'].resolve(node.arguments[0].value, _path2['default'].dirname(absolutePathToFile));
    ...
    const pathToRequiredFile = _requireRelative2['default'].resolve(node.source.value, _path2['default'].dirname(absolutePathToFile));

    const requiredFileContent = _fs2['default'].readFileSync(pathToRequiredFile, 'utf8');
    // eslint-disable-next-line no-use-before-define

    var _findExports2 = findExports(requiredFileContent, pathToRequiredFile);

    const named = _findExports2.named;
...
function findCommonJSExports(node, {
  ...
  const left = _node$expression.left,
...
function findDefinedNames(node, definedNames) {
  if (node.type === 'ExpressionStatement') {
    ...
    const left = _node$expression2.left,
...
function hasDefaultExport(nodes) {
  return nodes.some(node => {
    ...
    const left = _node$expression3.left,
...
function findRootNodes(ast) {
  ...
  try {
    ...
    //   }.call(this));
    //
    const callee = realRootNodes[0].expression.callee;
...
function findExports(data, absolutePathToFile) {
  ...
  const ast = (0, _parse2['default'])(data);
  ...
  let hasDefault = hasDefaultExport(rootNodes) || aliasesForExports.size > 1 || named.indexOf('default') !== -1;
  if (!hasDefault) {
