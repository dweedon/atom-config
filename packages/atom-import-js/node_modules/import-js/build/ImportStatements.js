...
const STYLES = Object.freeze([STYLE_SIDE_EFFECT, STYLE_IMPORT, STYLE_CONST, STYLE_VAR, STYLE_CUSTOM]);
...
// Order is significant here
const PATH_TYPES = Object.freeze([PATH_TYPE_CORE_MODULE, PATH_TYPE_PACKAGE, PATH_TYPE_NON_RELATIVE, PATH_TYPE_RELATIVE]);

const GROUPINGS_ARRAY = Object.freeze((0, _flattenDeep2['default'])(STYLES.map(style => PATH_TYPES.map(location => String(style) + ' ' + String(location)))));
...
function importStatementPathType(importStatement, packageDependencies, coreModules) {
  ...
  // (PATH_TYPE_PACKAGE).
  if (Array.from(packageDependencies).some(pkg => importStatement.path === pkg || importStatement.path.startsWith(String(pkg) + '/'))) {
...
function importStatementGroupIndex(importStatement, packageDependencies, coreModules) {
  ...
  const pathType = importStatementPathType(importStatement, packageDependencies, coreModules);
...
class ImportStatements {
   ...
   */
  find(callback) {
    const key = Object.keys(this.imports).find(key => callback(this.imports[key]));
    if (!key) {
      return undefined;
    }
  ...
  toArray() {
    ...
    this._toGroups().forEach(group => {
      group.forEach(importStatement => {
        const importStrings = importStatement.toImportStrings(maxLineLength, tab).map(importString => this.config.get('importStatementFormatter', {
  ...
  _toGroups() {
    ...
    let result = (0, _partition2['default'])(importsArray, importStatement => !importStatement.isParsedAndUntouched());
    result = (0, _flattenDeep2['default'])(result);

    if (this.config.get('sortImports')) {
      result = (0, _sortBy2['default'])(result, is => is.toNormalized());
    }

    result = (0, _uniqBy2['default'])(result, is => is.toNormalized());
    ...
    result.forEach(importStatement => {
      ...
      const groupIndex = importStatementGroupIndex(importStatement, packageDependencies, coreModules);
