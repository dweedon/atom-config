...
function normalizeNode(node, context) {
  ...
  const isAssignment = KEYS_USED_FOR_ASSIGNMENT.has(key) || key === 'key' && parent.parent.type === 'ObjectPattern' || key === 'left' && parent.type === 'AssignmentPattern' || key === 'elements' && parent.type === 'ArrayPattern' || key === 'value' && parent.parent.type === 'ObjectPattern' && parent.parent.parent.type === 'VariableDeclarator';
  if (isAssignment) {
    context.definedInScope.add(node.name);
  }

  const isReference = KEYS_USED_IN_REFERENCE_TO_OBJECTS.has(key) || key === 'key' && !parent.computed && parent.parent.type !== 'ObjectPattern';
...
function visitIdentifierNodes(rootAstNode, visitor, context = { definedInScope: new Set([]), key: 'root' }) {
