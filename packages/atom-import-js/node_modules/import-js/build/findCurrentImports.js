...
function convertToImportStatement(node, source) {
  if (node.type === 'ImportDeclaration') {
    const defaultSpecifier = node.specifiers.find(spec => spec.type === 'ImportDefaultSpecifier' || spec.type === 'ImportNamespaceSpecifier');
  ...
  if (node.type === 'VariableDeclaration') {
    ...
    const defaultImport = declaration.id.type === 'Identifier' ? declaration.id.name : undefined;

    const namedImports = declaration.id.type === 'ObjectPattern' ? declaration.id.properties.map(p => ({
...
function findCurrentImports(config, currentFileContent, ast) {
  ...
  ast.program.body.forEach((node, index) => {
    ...
    const importStatement = convertToImportStatement(node, currentFileContent.slice(node.start, node.end));
    if (!importStatement) {
